...
from windgram_product import WindgramData, build_windgram_data, windgram_leads
...
def parse_windgram_request(raw_text: str) -> ParsedWindgramRequest:
    ...
    if not text:
        raise ValueError("Не указана точка. Пример: /windgram Москва to=120")

    return ParsedWindgramRequest(text, run, lead_from, lead_to, step, top_hpa)
...
def format_windgram_caption(data: WindgramData) -> str:
    return (
        ...
        "Цвет — скорость ветра, стрелка — направление переноса, число — м/с."
    )
...
async def run_windgram_product(message, point: GeoPoint, parsed: ParsedWindgramRequest, gfs_semaphore) -> None:
    ...
    try:
        async with gfs_semaphore:
            header = (
                f"WINDGRAM | GFS {selected_run.date} {selected_run.cycle}Z | +{leads[0]}…+{leads[-1]} ч\n"
