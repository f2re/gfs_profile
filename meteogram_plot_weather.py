...
def _draw_precipitation(axis, x: np.ndarray, series: MeteogramSeries, tracked) -> None:
    ...
    if series.source.ensemble:
        ...
        for code, label, color in (
            ("precipitation_probability_0p1", "P ≥0,1 мм", COLORS["probability_0p1"]),
            ("precipitation_probability_1", "P ≥1 мм", COLORS["probability_1"]),
            ("precipitation_probability_5", "P ≥5 мм", COLORS["probability_5"]),
        ):
...
def _precipitation_class(rate: float, weather_code: int | None):
...
def _daily_precipitation_labels(axis, x, series, rates, tracked) -> list[Artist]:
    ...
    for day_number, day in enumerate(dict.fromkeys(dates)):
        ...
        total = float(np.nansum(raw[indices]))
        if total < 0.1:
            continue
        maximum_rate = max(
        ...
        drizzle_hours = sum(
            ...
            if (
                (np.isfinite(weather[index]) and int(round(weather[index])) in DRIZZLE_CODES)
        ...
        if sum(code in THUNDER_CODES for code in wet_codes) >= required_codes:
            description = "гроза / ливень"
        elif sum(code in FREEZING_CODES for code in wet_codes) >= required_codes:
            description = "переохлаждённые осадки"
        elif sum(code in SNOW_CODES for code in wet_codes) >= required_codes:
            description = "снег"
        elif maximum_rate >= 10.0:
            description = "ливень"
        elif persistent_drizzle:
            description = "длительная морось"
        elif total < 1.0:
        ...
        artist = axis.text(
            ...
            f"{total:.1f} мм/сут · {description}".replace(".", ","),
            transform=transform,
            ha="center",
            va="bottom",
            fontsize=5.9,
