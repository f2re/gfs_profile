...
4. Показывает параметры кнопками: для `/map` сроки `+24/+48/+72/+96`, для анимации и PNG-серии диапазон до `+96` с шагом `6 ч`, и только две подложки `Полная/Базовая`; для `/windgram` верхний уровень фиксирован `top=500`, отдельная кнопка выбора не показывается.
