...
/map Краснодар from=0 to=24 step=3 mode=series
  серия отдельных PNG-карт: +0, +3, +6, ... +24
...
Если Overpass недоступен, карта строится на простой белой подложке и не падает. На самой карте указано: `GFS 0.25 — модель, не радар и не наблюдения`.
