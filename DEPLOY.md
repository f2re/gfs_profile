...
Для PDF метеограммы отдельный `libreoffice-writer` больше не является обязательным системным пакетом: основной PDF-рендер работает через уже установленный Matplotlib. `LIBREOFFICE_BIN` можно оставить только как необязательный резервный конвертер. После обновления достаточно штатного `bash deploy_telegram_bot.sh`.
