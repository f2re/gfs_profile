...
class MessengerWebhookService:
    ...
    def from_env(cls, router: MessengerRouter | None = None) -> "MessengerWebhookService":
        ...
        max_gateway = MaxGateway(MaxApiClient(max_token)) if max_token else None
        vk_gateway = VkGateway(
            VkApiClient(vk_token, api_version=os.getenv("VK_API_VERSION", "5.199"))
        ) if vk_token else None
        return cls(
            router,
            max_gateway=max_gateway,
            vk_gateway=vk_gateway,
            max_secret=os.getenv("MAX_WEBHOOK_SECRET"),
        ...
        async def vk_webhook(request: Request):
            ...
            if payload.get("type") == "confirmation":
                if not self.vk_confirmation_code:
                    raise HTTPException(status_code=503, detail="VK confirmation code is not configured")
                return PlainTextResponse(self.vk_confirmation_code)
            if self.vk_gateway is None:
                raise HTTPException(status_code=503, detail="VK gateway is not configured")
