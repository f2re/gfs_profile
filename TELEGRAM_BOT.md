...
Команды `/aero`, `/skewt`, `/windgram`, `/cloudgram` и `/map` без параметров запускают кнопочный выбор точки и параметров. В reply-клавиатуре последние локации показываются красивыми сокращёнными подписями; при нажатии бот не геокодирует видимый текст с эмодзи/многоточием, а берёт полную сохранённую точку из сессии.
